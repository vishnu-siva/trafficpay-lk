  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-blue-900">Access control</p>
          <h1 className="mt-1.5 text-3xl font-black text-slate-950">Officers</h1>
        </div>
      ...
      {showForm && (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-md sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select name="district" value={form.district} onChange={handleChange} className="field">
                {DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>

            {error && (
              <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 sm:col-span-2">
                {error}
              </div>
            )}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-md">
        <div className="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/70 px-5 py-4">
          ...
          <p className="text-sm font-semibold text-slate-400">{officers.length.toLocaleString()} shown</p>
        </div>
        {loadingOfficers ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {officers.map(officer => (
                  <tr key={officer.officerId || officer.badgeNumber} className="transition-colors duration-100 hover:bg-blue-50/40">
                    <td className="whitespace-nowrap px-4 py-3 font-semibold text-slate-900">{officer.fullName || officer.name || '-'}</td>
                    <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-slate-500">{officer.badgeNumber || '-'}</td>
