...
export default function FinesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs font-bold uppercase tracking-widest text-blue-900">Case operations</p>
        <h1 className="mt-1.5 text-3xl font-black text-slate-950">Fines</h1>
      </div>
      ...
      <div className="surface flex flex-wrap items-center gap-3 rounded-xl p-4">
        ...
        />
        <button onClick={fetchFines} className="btn-primary">Search</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-md">
          <div className="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/70 px-5 py-4">
            ...
            <p className="text-sm font-semibold text-slate-400">{fines.length.toLocaleString()} shown</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {fines.map(fine => (
                  <tr key={fineKey(fine)} className="transition-colors duration-100 hover:bg-blue-50/40">
                    <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-slate-500">{fine.referenceNumber || '-'}</td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold ${STATUS_STYLES[fine.status] || 'bg-slate-100 text-slate-700'}`}>
                        {fine.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {fine.status === 'PENDING' && (
                        <button
                          onClick={() => setCancelId(fine.fineId || fine.id)}
                          className="rounded-lg px-2.5 py-1 text-xs font-bold text-rose-700 transition-all duration-150 hover:bg-rose-50 hover:shadow-sm"
                        >
                          Cancel
                        </button>
                      )}
      ...
      {cancelId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-200/80">
